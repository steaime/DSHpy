...
def GenerateROIgrid(px_coords, GridShape, coord_limits=None, common_mask=None):
    ...
    Parameters
    ----------
    px_coords :   pixel coordinates, as generated by PixelCoordGrid
...
def GenerateGridAndMasks(coords, shape, center=None, common_mask=None, binary_res=False, coordsystem='polar', ref_angle=0):
    """Generate a list of regions of interest, labelled either in the form of binary images, 
    each one with 0s everywhere and 1 inside the region of interest,
...
class ROIproc():
    ...
    def __init__(self, MIin, ROImasks, ROIcoords=None, imgTimes=None, expTimes=[1]):
        """
        Initialize SALS
        ...
        ROIcoords : list of ROI coordinates, one element per ROI. 
                    Each element can be a scalar or a tuple
    ...
    def SetROIs(self, ROIs, CommonMask=None, ROIcoords=None, ROIcoord_names=None):
        '''
        Sets ROIs from integer mask
        '''
    ...
    def SetROImasks(self, ROImasks, BoundingBoxMargin=0, ROIcoords=None, ROIcoord_names=None):
        ...
        ROIcoord_names : list of str: coordinate names
                    If None, ROIcoord_names = [coord0, coord1, ...]
        ...
        globalBB = FindBoundingBoxROI(self.ROI_masks, BoundingBoxMargin)
    ...
    def GetImage(self, image_idx, buffer=None):
        return self.MIinput.GetImage(image_idx, cropROI=self.CropROIbb, buffer=buffer)
    ...
    def SaveIavg(self, SaveFolder, Iavg, AllExpData=None):
        ...
        np.savetxt(os.path.join(SaveFolder, 'ROIcoords.dat'), np.append(self.ROIcoords, roi_norms, axis=1), 
                   header=ROIhdr_str+self.txt_delim+'norm', **self.savetxt_kwargs)

        MI.WriteBinary(os.path.join(SaveFolder, 'ROI_mask.raw'), self.ROIs, 'i')
        
        str_hdr_avg = ROIhdr_str + ''.join([self.txt_delim+'t{0:.2f}'.format(self.imgTimes[i])
        ...
        if AllExpData is not None:
            ...
            np.savetxt(os.path.join(SaveFolder, 'exptimes.dat'), np.append(self.ROIcoords[:,:2], BestExptime_Idx.T, axis=1), 
                       header=str_hdr_avg, **self.savetxt_kwargs)
            str_hdr_raw = ROIhdr_str + ''.join([self.txt_delim+'t{0:.2f}_e{1:.3f}'.format(self.imgTimes[i], self.expTimes[i%len(self.expTimes)])
    ...
    def doDLS(self, saveFolder, lagtimes, reftimes='all', no_buffer=False, force_SLS=True, save_transposed=False):
        ...
        logging.info('ROIproc.doDLS Analysis started! Input data is {0} images ({1} times, {2} exposure times)'.format(self.ImageNumber(), self.NumTimes(), self.NumExpTimes()))
        ...
        logging.info('Analysis will resolve {0} ROIs and DLS will be performed on {1} reference times and {2} lagtimes. Output will be saved in folder {3}'.format(self.CountROIs(), len(DLS_reftimes), DLS_lagnum, saveFolder))
        logging.info('Now starting with SLS...')
                
        if len(DLS_reftimes)<1:
            logging.warn('WARNING: at least 1 reference time needed for DLS (' + str(len(DLS_reftimes)) + ' given). DLS aborted')
            return None
        ...
        if save_transposed:
            logging.info('DLS analysis completed. Transposed result saved (no g2-1 function will be averaged).')
        else:
