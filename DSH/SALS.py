        ...
        if DLS_lags is None:
            ...
            for e in range(self.NumExpTimes()):
                ...
                if reftimes in ['auto', 'all'] and lagtimes=='auto':
                    ...
                    if self.DebugMode:
                        corr_idx = np.nan * np.ones((len(DLS_reftimes), DLS_lagnum, 2), dtype=float)
                    
                    # This cycle is key for the upcoming normalization step, don't incorporate in the main loop
                    for ref_tidx in range(len(DLS_reftimes)):
                        cI[:,ref_tidx,0] = np.subtract(np.divide(ISQavg[DLS_reftimes[ref_tidx],:], np.square(ROIavgs_allExp[DLS_reftimes[ref_tidx],e,:])), 1)
                    logging.debug('Contrast (d0) processed')
                        
                    for ref_tidx in range(len(DLS_reftimes)):

                        if lagtimes=='all':
                            cur_lagtimes = np.append(np.asarray([0]), np.arange(self.NumTimes())-DLS_reftimes[ref_tidx])
                        else:
                            cur_lagtimes = DLS_lags[DLS_lags < self.NumTimes()-DLS_reftimes[ref_tidx]]
                        ...
                        if self.DebugMode:
                            logging.debug('Shape of ROIaverageProduct output: {0}. max lagtime (total): {1}, max lagtime (current): {2}, max img time: {3}'.format(IXavg.shape, 
                                            DLS_lags[-1], cur_lagtimes[-1], DLS_reftimes[ref_tidx]+cur_lagtimes[-1]))
                        
                        for lidx in range(1,len(cur_lagtimes)):
                            # 'classic' cI formula
                            cI[:,ref_tidx,lidx] = np.subtract(np.divide(IXavg[lidx,:], np.multiply(ROIavgs_allExp[DLS_reftimes[ref_tidx],e,:],
                                                                                                   ROIavgs_allExp[DLS_reftimes[ref_tidx]+cur_lagtimes[lidx],e,:])), 1)
                            # d0 normalization
                            # if skipping reference times here and there, calculate contrast for t+tau
                            if reftimes in ['auto', 'all']:
                                cur_norm =  0.5 * np.add(cI[:,ref_tidx,0], cI[:,ref_tidx+cur_lagtimes[lidx],0])
                            else:
                                cur_norm = 0.5 * np.add(cI[:,ref_tidx,0], 
                                                        np.subtract(np.divide(ISQavg[DLS_reftimes[ref_tidx]+cur_lagtimes[lidx],:], 
                                                                              np.square(ROIavgs_allExp[DLS_reftimes[ref_tidx]+cur_lagtimes[lidx],e,:])), 1))
                            cI[:,ref_tidx,lidx] = np.divide(cI[:,ref_tidx,lidx], cur_norm)
                                
                        logging.debug('Reference time {0}/{1} processed (image #{2}, {3} lagtimes)'.format(ref_tidx, len(DLS_reftimes), DLS_reftimes[ref_tidx], len(cur_lagtimes)))
